...
assertEquals([[0, 6], [0, 4], [0, 4]], Array.from(nested.indices));
assertEquals(bytes(eAcute + eCircumflex), bytes(nested[1]));
assertEquals(bytes(nested[1]), bytes(nested[2]));
assertNull(/(([A-C\u00e9-\u00eb]{2}))xy/du.exec(eAcute + 'xy'));

const triple =
    /x=((([A-C\u00e9-\u00eb]{2})))y/du.exec('x=' + eAcute + eCircumflex + 'y');
assertNotNull(triple);
assertEquals([[0, 7], [2, 6], [2, 6], [2, 6]], Array.from(triple.indices));

const named = /id=((?<value>[A-C\u00e9-\u00eb]{3}))xy/du.exec(
    'id=A' + eAcute + eCircumflex + 'xy');
assertNotNull(named);
for (let capture = 1; capture <= 2; ++capture) {
  assertEquals(bytes('A' + eAcute + eCircumflex), bytes(named[capture]));
  assertEquals([3, 8], named.indices[capture]);
}
assertEquals(bytes(named[2]), bytes(named.groups.value));
assertEquals(named.indices[2], named.indices.groups.value);

const eightScalars = 'A' + eAcute + 'BC' + eCircumflex + 'ABC';
const upperBound =
    /p=(([A-C\u00e9-\u00eb]{8}))z/du.exec('p=' + eightScalars + 'z');
assertNotNull(upperBound);
assertEquals([[0, 13], [2, 12], [2, 12]], Array.from(upperBound.indices));

const lazySpelling =
    /k=(([A-C\u00e9-\u00eb]{2}?))xy/du.exec('k=' + eAcute + eCircumflex + 'xy');
assertNotNull(lazySpelling);
assertEquals([[0, 8], [2, 6], [2, 6]], Array.from(lazySpelling.indices));
...
assertEquals([[1, 9], [3, 7], [3, 7]], Array.from(malformedMatch.indices));
...
for (let capture = 1; capture <= 2; ++capture) {
  assertEquals([[2, 6], [10, 13]], all.map(match => match.indices[capture]));
}
...
assertEquals(
    [[
      ...
      bytes(eAcute + eCircumflex), bytes(eAcute + eCircumflex)
    ]],
    replacementCalls);
...
assertEquals([[0, 4], [0, 4], [0, 4]], Array.from(complete.indices));
