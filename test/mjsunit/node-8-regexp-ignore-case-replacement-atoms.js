...
// Scope: replacement literal atoms and byte-coordinate consumers.
'use strict';
...
(() => {
  ...
  function makeApi(source, flags, slow) {
    const wrap = re => {
      ...
      re.exec = function(subject) {
        if (++calls > 64) throw Error('REPLACEMENT_ATOMS_BOUNDED_EXEC');
  ...
  for (const slow of [false, true]) {
    for (const grammar of ['', 'u', 'v']) {
      ...
      const source = '(' + string(REPLACEMENT) + '*)';
      ...
      for (let repeat = 0; repeat < 2; ++repeat) {
        ...
        equal(tag + '-callback-result', units(input.replace(re, (m, capture, offset, original) => {
          if (calls.length > 16) throw Error('REPLACEMENT_ATOMS_BOUNDED_CALLBACK');
  ...
  equal('fixed-case-count', cases, 1968);
  equal('fixed-check-count', checks + 1, 15890);
  emit(encode({kind: 'replacement-atoms-profile', profile, actualWidth: width}));
  emit(encode({kind: 'ignore-case-replacement-atoms', profile,
    cases, checks, expectedCases: 1968, expectedChecks: 15890,
  ...
  emit(encode({kind: 'excluded-replacement-atoms-observations', cases: observations.length,
    gating: false, observations}));
  if (failureCount !== 0) throw Error('REPLACEMENT_ATOMS_ORACLE: ' + failureCount);
})();
