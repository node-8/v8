...
assertNotNull(nestedOpen);
assertEquals(bytes(eAcute + cjk), bytes(nestedOpen[0]));
assertEquals(bytes(nestedOpen[0]), bytes(nestedOpen[1]));
for (let capture = 2; capture <= 3; ++capture) {
  assertEquals(bytes(cjk), bytes(nestedOpen[capture]));
  assertEquals([2, 5], nestedOpen.indices[capture]);
}
assertEquals([0, 5], nestedOpen.indices[1]);
assertEquals(nestedOpen.indices[3], nestedOpen.indices.groups.part);
...
const malformed = /((([^^]){2,3}?))/du.exec(raw(0x61, 0xe2, 0x82, 0x62));
...
assertEquals([0xe2, 0x82], bytes(malformed[3]));
assertEquals([[0, 3], [0, 3], [0, 3], [1, 3]],
...
function expectedLazy(input, start, min) {
  ...
  for (let iteration = 0; iteration < min; ++iteration) {
  ...
  return {
    ...
    capture: min === 0 ? undefined : input.slice(captureStart, position),
    captureStart,
  };
}
...
for (const input of corpora) {
  ...
  for (let start = 0; start <= input.length; ++start) {
    for (const {regexp, min, outer, inner, names} of lazyPatterns) {
