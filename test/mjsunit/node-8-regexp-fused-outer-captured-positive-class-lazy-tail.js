...
assertEquals(bytes(eAcute + 'x'), bytes(minimum[0]));
assertEquals(bytes(eAcute), bytes(minimum[1]));
assertEquals([[0, 3], [0, 2]], Array.from(minimum.indices));

assertNull(/([A-C\u00e9-\u00eb]{2,3}?)x/du.exec(eAcute + 'x'));

const expansion = /([A-C\u00e9-\u00eb]{1,3}?)x/du.exec('A' + eAcute + 'x');
assertNotNull(expansion);
assertEquals(bytes('A' + eAcute), bytes(expansion[1]));
assertEquals([[0, 4], [0, 3]], Array.from(expansion.indices));

const nested = /((?<run>[A-C\u00e9-\u00eb]{1,3}?))x/du.exec('A' + eAcute + 'x');
assertNotNull(nested);
for (let capture = 1; capture <= 2; ++capture) {
  assertEquals(bytes('A' + eAcute), bytes(nested[capture]));
  assertEquals([0, 3], nested.indices[capture]);
}
...
assertEquals([[1, 4], [1, 3]], Array.from(malformedMatch.indices));
assertEquals(4, malformed.lastIndex);

const all = Array.from((eAcute + 'xA' + eCircumflex + 'x')
                           .matchAll(/([A-C\u00e9-\u00eb]{1,3}?)x/dgu));
assertEquals([[0, 3], [3, 7]], all.map(match => match.indices[0]));
assertEquals([[0, 2], [3, 6]], all.map(match => match.indices[1]));

const replacementCalls = [];
assertEquals(
    'Y',
    ('A' + eAcute + 'x')
        .replace(/([A-C\u00e9-\u00eb]{1,3}?)x/gu, (match, run, offset) => {
          replacementCalls.push([offset, bytes(match), bytes(run)]);
          return 'Y';
        }));
assertEquals(
    [[0, bytes('A' + eAcute + 'x'), bytes('A' + eAcute)]], replacementCalls);
...
function expectedSearch(input, start, pattern) {
  for (let candidate = start; candidate < input.length; ++candidate) {
    ...
    for (let repeated = pattern.minimum; repeated < ends.length; ++repeated) {
...
// The accepted greedy endpoint order remains unchanged.
const greedy = /([A-Cx\u00e9-\u00eb]{1,3})x/du.exec('Axx');
assertEquals([[0, 3], [0, 2]], Array.from(greedy.indices));
