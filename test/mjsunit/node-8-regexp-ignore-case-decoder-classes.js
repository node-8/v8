...
// Scope: decoder-sensitive ignore-case classes and byte-coordinate consumers.
'use strict';
...
(() => {
  ...
  const families = [
    ...
    ...malformed.map((value, i) => ['malformed-' + i, '[\\ufffd]', value]),
  ];
  ...
  rows.push(
    ...
    row('replacement-no-interior', '([\\ufffd])', EC, null),
  ...
  for (const [i,bad] of malformed.entries()) {
    rows.push(
      row('replacement-tail-' + i, '([\\ufffd])([' + string(E) + '])',
  ...
  function makeApi(source, flags, slow) {
    const wrap = re => {
      ...
      re.exec = function(subject) {
        if (++calls > 64) throw Error('DECODER_CLASSES_BOUNDED_EXEC');
    ...
    for (const grammar of ['', 'u', 'v']) {
      ...
      for (let repeat = 0; repeat < 2; ++repeat) {
        ...
        equal(tag + '-callback-result', units(input.replace(re, (m, capture, offset, original) => {
          if (calls.length > 16) throw Error('DECODER_CLASSES_BOUNDED_CALLBACK');
        ...
        for (const [name, subject, start, tail] of [
          ['valid', join(Z0,DL,bang), 0, join(Z0,DL,bang)],
          ['interior', join(Z0,DL,bang), 1,
            fixture([...Z0.bytes.slice(1),...DL.bytes,...bang.bytes], [...DL.stock,...bang.stock])],
          ['malformed', join(ZTRUNC,FF,CONT,bang), 0, join(ZTRUNC,FF,CONT,bang)],
          ['surrogate', join(H,bang), 0, join(H,bang)],
        ]) {
          const next = makeApi(source, 'dgi' + grammar, slow);
          next.lastIndex = start;
          const end = selected(subject).length;
          const wanted = [
            [[selected(tail),selected(tail)],start,[[start,end],[start,end]]],
            [[[],[]],end,[[end,end],[end,end]]],
          ];
          equal(tag + '-' + name, boundedAll(string(subject),next), [wanted,true], tag);
  ...
  equal('fixed-case-count', cases, 1752);
  equal('fixed-check-count', checks + 1, 14258);
  emit(encode({kind: 'decoder-classes-profile', profile, actualWidth: width}));
  emit(encode({kind: 'ignore-case-decoder-classes', profile,
    cases, checks, expectedCases: 1752, expectedChecks: 14258,
  ...
  emit(encode({kind: 'excluded-decoder-classes-observations', cases: observations.length,
    gating: false, observations}));
  if (failureCount !== 0) throw Error('DECODER_CLASSES_ORACLE: ' + failureCount);
})();
