...
assertEquals([bytes(cjk), undefined, bytes(emoji), undefined],
...
const positiveRange = /([é-ë])*/du.exec(eAcute + eCircumflex + 'x');
...
assertEquals([0, 4], positiveRange.indices[0]);
assertEquals([2, 4], positiveRange.indices[1]);
...
continuation.lastIndex = 5;
const emptySticky = continuation.exec(eAcute + cjk + '\n');
assertNotNull(emptySticky);
assertEquals('', emptySticky[0]);
assertEquals(undefined, emptySticky[1]);
assertEquals([5, 5], emptySticky.indices[0]);
...
function expectedStar(input, start) {
  if (start >= input.length || input[start] === 0x0a) {
    return {full: [], capture: undefined, captureStart: -1};
  }
  let position = start;
...
const starPatterns = [
  {regexp: /([^\n])*/duy, captures: [1]},
  {regexp: /(?<part>[^\n]){0,}/duy, captures: [1], name: 'part'},
  {regexp: /((([^\n])))*/duy, captures: [1, 2, 3]},
];
for (const input of corpora) {
  const value = raw(...input);
  for (let start = 0; start <= input.length; ++start) {
    for (const {regexp, captures, name} of starPatterns) {
      const expected = expectedStar(input, start);
