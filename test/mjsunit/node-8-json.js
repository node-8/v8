...
for (const replacer of [undefined, identityReplacer]) {
  ...
  for (const malformedBytes of [
    ...
    assertEquals(
        JSON.stringify(malformed.normalize()),
        JSON.stringify(malformed, replacer));
  }
}
...
assertEquals(
    [0xf0, 0x9f, 0x98, 0x80], bytes(JSON.parse('"\\ud83d\\ude00"')));
...
const rawMalformedJson = String.fromCharCode('"'.charCodeAt(0), 0x80,
                                              '"'.charCodeAt(0));
const rawMalformedValue = JSON.parse(rawMalformedJson);
assertEquals([0x80], bytes(rawMalformedValue));
assertEquals(
    JSON.stringify(rawMalformedValue.normalize()),
    JSON.stringify(rawMalformedValue));

const value = {latin: '\u00e9', cjk: '\u4e2d\u6587', emoji: '\ud83d\ude00'};
assertEquals(value, JSON.parse(JSON.stringify(value)));

const surrogateKey = String.fromCodePoint(0xd800);
const keyed = {[surrogateKey]: 1};
