...
const bodyOnly =
    /([A-C\u00e9-\u00eb]){2}xy/du.exec(eAcute + eCircumflex + 'xy');
assertNotNull(bodyOnly);
assertEquals([[0, 6], [2, 4]], Array.from(bodyOnly.indices));
assertEquals(bytes(eCircumflex), bytes(bodyOnly[1]));
assertNull(/([A-C\u00e9-\u00eb]){2}xy/du.exec(eAcute + 'xy'));

const mixed = /(([A-C\u00e9-\u00eb]){2})xy/du.exec(eAcute + eCircumflex + 'xy');
assertNotNull(mixed);
assertEquals([[0, 6], [0, 4], [2, 4]], Array.from(mixed.indices));
assertEquals(bytes(eAcute + eCircumflex), bytes(mixed[1]));
assertEquals(bytes(eCircumflex), bytes(mixed[2]));

const namedBody = /id=((?<last>[A-C\u00e9-\u00eb])){3}xy/du.exec(
    'id=A' + eAcute + eCircumflex + 'xy');
assertNotNull(namedBody);
assertEquals([[0, 10], [6, 8], [6, 8]], Array.from(namedBody.indices));
assertEquals(bytes(eCircumflex), bytes(namedBody[1]));
assertEquals(bytes(namedBody[1]), bytes(namedBody.groups.last));
assertEquals(namedBody.indices[1], namedBody.indices.groups.last);

const nestedMixed =
    /p=((([A-C\u00e9-\u00eb])){2})z/du.exec('p=' + eAcute + eCircumflex + 'z');
...
assertEquals([[0, 8], [2, 6], [4, 6]], Array.from(lazySpelling.indices));
