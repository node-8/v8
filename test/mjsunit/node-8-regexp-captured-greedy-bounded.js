...
assertEquals(bytes(eCircumflex), bytes(ordinary[1]));
assertEquals([[0, 12], [10, 12]], Array.from(ordinary.indices));
...
const outer = /(?<run>[^\n]{2,5})/du.exec(subject);
assertNotNull(outer);
assertEquals(bytes(subject), bytes(outer.groups.run));
assertEquals([0, 12], outer.indices.groups.run);
...
assertEquals([[0, bytes(subject), bytes(eCircumflex)]], replacementCalls);
...
assertEquals(bytes('a'), bytes(continuation[1]));
assertEquals([[1, 6], [5, 6]], Array.from(continuation.indices));
...
assertEquals([[0, 4], [3, 4]], Array.from(malformed.indices));
...
const boundPatterns = [
  {regexp: /([^\n]){2,5}/duy, min: 2, max: 5, captures: [1]},
  {
    regexp: /(?<part>[^\n]){1,2}/duy,
    min: 1,
    max: 2,
  ...
  {regexp: /((([^\n]))){0,3}/duy, min: 0, max: 3, captures: [1, 2, 3]},
];
for (const input of corpora) {
  const value = raw(...input);
  for (let start = 0; start <= input.length; ++start) {
    for (const {regexp, min, max, captures, name} of boundPatterns) {
