...
// Fixed stock/byte oracles for local ignore-case disable and mixed scopes.
'use strict';
...
(() => {
  ...
  const e=string(E),c=string(C),K=fixture([0xe2,0x84,0xaa],[0x212a]);
  const S=fixture([0xc5,0xbf],[0x17f]),triple=join(EC,E,EC),double=join(E,E);
  rows.push(
    local('local-simple','^(?-i:('+e+'))$',E,[E,E],[[0,2],[0,2]],[[0,1],[0,1]]),
    local('local-negative','^(?-i:('+e+'))$',EC,null),
    local('folded-siblings','^'+e+'(?-i:('+e+'))'+e+'$',triple,[triple,E],
      [[0,6],[2,4]],[[0,3],[1,2]]),
    local('sensitive-child-negative','^'+e+'(?-i:('+e+'))'+e+'$',join(EC,EC,EC),null),
    local('ascii-folded-siblings','^a(?-i:(b))c$',ascii('AbC'),[ascii('AbC'),ascii('b')],
      [[0,3],[1,2]],[[0,3],[1,2]]),
    local('ascii-child-negative','^a(?-i:(b))c$',ascii('ABC'),null),
    local('two-scopes','^(?-i:('+e+'))-(?-i:('+e+'))$',join(E,ascii('-'),E),
      [join(E,ascii('-'),E),E,E],[[0,5],[0,2],[3,5]],[[0,3],[0,1],[2,3]]),
    local('outside-repeat','^(?-i:('+e+'))+$',double,[double,E],
      [[0,4],[2,4]],[[0,2],[1,2]]),
    local('outside-star','(?-i:('+e+'))*',double,[double,E],
      [[0,4],[2,4]],[[0,2],[1,2]]),
    local('reenable','^(?-i:'+e+'(?i:('+e+'))'+e+')$',join(E,EC,E),[join(E,EC,E),EC],
      [[0,6],[2,4]],[[0,3],[1,2]]),
    local('reenable-prefix-negative','^(?-i:'+e+'(?i:('+e+'))'+e+')$',join(EC,EC,E),null),
    local('disable-in-local-enable','^(?i:'+e+'(?-i:('+e+'))'+e+')$',triple,[triple,E],
      [[0,6],[2,4]],[[0,3],[1,2]],'all',''),
    local('nested-same-disable','^(?-i:(?-i:('+e+')))$',E,[E,E],
      [[0,2],[0,2]],[[0,1],[0,1]]),
    local('outside-ms','(?ms:^(?-i:(.'+e+'))$)',join(ascii('\n'),E),
      [join(ascii('\n'),E),join(ascii('\n'),E)],[[0,3],[0,3]],[[0,2],[0,2]]),
    local('combined-ms-disable','(?ms-i:^(.'+e+'))$',join(ascii('\n'),E),
      [join(ascii('\n'),E),join(ascii('\n'),E)],[[0,3],[0,3]],[[0,2],[0,2]]),
    local('dotall-restored-negative','^(?s-i:(.'+e+')).$',join(ascii('\n'),E,ascii('\n')),null),
    local('outside-folded-class','^['+e+'](?-i:('+e+'))$',join(EC,E),[join(EC,E),E],
      [[0,4],[2,4]],[[0,2],[1,2]]),
    local('outside-class-child-negative','^['+e+'](?-i:('+e+'))$',join(EC,EC),null),
    local('sensitive-negated','^(?-i:([^a]+))$',join(ascii('A'),C),
      [join(ascii('A'),C),join(ascii('A'),C)],[[0,4],[0,4]],[[0,2],[0,2]]),
    local('decoder-folded-tail','^(?-i:([^a]))'+e+'$',join(raw([0x80]),EC),
      [join(raw([0x80]),EC),raw([0x80])],[[0,3],[0,1]],[[0,2],[0,1]]),
    local('outside-folded-boundaries','^\\b(?-i:('+string(K)+'))\\b$',K,[K,K],
      [[0,3],[0,3]],[[0,1],[0,1]],'unicode'),
    local('inside-sensitive-boundaries','^(?-i:\\B('+string(K)+')\\B)$',K,[K,K],
      [[0,3],[0,3]],[[0,1],[0,1]]),
    local('outside-nonboundary-negative','^\\B(?-i:('+string(K)+'))\\B$',K,[K,K],
      [[0,3],[0,3]],[[0,1],[0,1]],'stock-legacy'),
    local('named-mixed','^(?<a>'+e+')(?-i:(?<b>'+e+'))$',join(EC,E),[join(EC,E),EC,E],
      [[0,4],[0,2],[2,4]],[[0,2],[0,1],[1,2]],'all','i',0,{a:1,b:2}),
    local('nested-nullable-boundary','(?-i:(?i:\\B()))',E,[empty,empty],
      [[0,0],[0,0]],[[0,0],[0,0]]),
    local('nested-explicit-origin','(?-i:(?i:\\B()))',E,[empty,empty],
      [[1,1],[1,1]],[[0,0],[0,0]],'all','ig',byte?1:0)
  );
  ...
  function makeApi(source, flags, slow) {
    ...
    const wrap = re => {
      ...
      re.exec=function(subject) {
        if(++calls>64) throw Error('LOCAL_DISABLE_I_BOUNDED_EXEC');
  ...
  for(const grammar of ['', 'u', 'v']) {
    const boundary=byte?[9,10]:[5,6];
    const nonboundary=byte?[0,3,4,6,7]:[0,1,2,3,4];
    ...
    const apiRows=[
      ...
      ['consuming','('+string(EC)+')\\b',byte?[[7,9]]:[[4,5]]],
    ];
      ...
      for(const slow of [false,true]) {
        ...
        for(let repeat=0;repeat<2;++repeat) {
          ...
          equal(tag+'callback-result',units(input.replace(re,(m,capture,offset,original)=>{
            if(calls.length>16) throw Error('LOCAL_DISABLE_I_BOUNDED_CALLBACK');
          ...
          equal(tag+'split',input.split(makeApi(source,'i'+grammar,slow)).map(units),split,tag);
  ...
  equal('fixed-case-count',cases,810);
  equal('fixed-check-count',checks+1,6968);
  emit(encode({kind:'local-disable-i-profile',profile,actualWidth:width}));
  emit(encode({kind:'local-disable-i',profile,
    cases,checks,expectedCases:810,expectedChecks:6968,
  ...
  if(failureCount!==0) throw Error('LOCAL_DISABLE_I_ORACLE: '+failureCount);
})();
