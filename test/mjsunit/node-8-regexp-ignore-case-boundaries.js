...
// Scope: Unicode-folded word assertions with fixed stock and byte oracles.
'use strict';
...
(() => {
  ...
  function makeApi(source, flags, slow) {
    const wrap = re => {
      ...
      re.exec=function(subject) {
        if(++calls>64) throw Error('IC_BOUNDARIES_BOUNDED_EXEC');
      ...
      for(const slow of [false,true]) {
        ...
        for(let repeat=0;repeat<2;++repeat) {
          ...
          equal(tag+'callback-result',units(input.replace(re,(m,capture,offset,original)=>{
            if(calls.length>16) throw Error('IC_BOUNDARIES_BOUNDED_CALLBACK');
  ...
  equal('fixed-case-count',cases,900);
  equal('fixed-check-count',checks+1,7688);
  emit(encode({kind:'ignore-case-boundaries-profile',profile,actualWidth:width}));
  emit(encode({kind:'ignore-case-boundaries',profile,
    cases,checks,expectedCases:900,expectedChecks:7688,
  ...
  if(failureCount!==0) throw Error('IC_BOUNDARIES_ORACLE: '+failureCount);
})();
