...
// Scope: local multiline flags with fixed stock and byte oracles.
'use strict';
...
(() => {
  ...
  const rootRows=rows.splice(0);
  for(const item of rootRows) {
    rows.push(
      {...item,id:'enable-'+item.id,source:'(?m:'+item.source+')',
        extraFlags:item.extraFlags.replace('m','')},
      {...item,id:'nested-enable-'+item.id,source:'(?-m:(?m:'+item.source+'))'}
    );
  }
  for(const fold of [false,true]) {
    const part=fold?EC:E, i=fold?'i':'', atom=string(E);
    rows.push(
      row('disable-whole'+fold,'(?-m:^('+atom+')$)',part,[part,part],
        [[0,2],[0,2]],[[0,1],[0,1]],'all','m'+i),
      row('nested-disable-whole'+fold,'(?m:(?-m:^('+atom+')$))',part,[part,part],
        [[0,2],[0,2]],[[0,1],[0,1]],'all',i)
    );
    for(const [label,sep,newline] of separators) {
      const b=sep.bytes.length, s=sep.stock.length;
      rows.push(
        row('disabled-line'+label+fold,'(?-m:^('+atom+')$)',join(sep,part,sep),
          null,undefined,undefined,'all','m'+i),
        row('sibling-input-end'+label+fold,'(?m:^('+atom+'))$',join(sep,part,sep),
          null,undefined,undefined,'all',i),
        row('sibling-input-start'+label+fold,'^(?m:('+atom+')$)',join(part,sep),
          newline?[part,part]:null,[[0,2],[0,2]],[[0,1],[0,1]],'all',i),
        row('sibling-line-end'+label+fold,'(?-m:^('+atom+'))$',join(part,sep),
          newline?[part,part]:null,[[0,2],[0,2]],[[0,1],[0,1]],'all','m'+i),
        row('sibling-line-start'+label+fold,'^(?-m:('+atom+')$)',join(sep,part),
          newline?[part,part]:null,[[b,b+2],[b,b+2]],[[s,s+1],[s,s+1]],'all','m'+i)
      );
    }
    const line=join(ascii('\n'),part);
    for(const [label,sep] of separators.slice(0,5)) {
      const b=sep.bytes.length,s=sep.stock.length;
      rows.push(
        row('mixed-ms'+label+fold,'(?ms:^(.'+atom+')$)',join(sep,line,sep),
          [line,line],[[b,b+3],[b,b+3]],[[s,s+2],[s,s+2]],'all',i),
        row('mixed-m-minus-s'+label+fold,'(?m-s:^(.'+atom+')$)',join(sep,line,sep),
          null,undefined,undefined,'all','s'+i)
      );
    }
    rows.push(
      row('mixed-s-minus-m-whole'+fold,'(?s-m:^(.'+atom+')$)',line,[line,line],
        [[0,3],[0,3]],[[0,2],[0,2]],'all','m'+i),
      row('mixed-s-minus-m-line'+fold,'(?s-m:^(.'+atom+')$)',join(LS,line,PS),
        null,undefined,undefined,'all','m'+i)
    );
  }
  rows.push(row('case-sensitive-negative','(?m:^'+string(E)+'$)',EC,
    null,undefined,undefined,'all',''));
  ...
  function makeApi(source, flags, slow) {
    const wrap = re => {
      ...
      re.exec = function(subject) {
        if (++calls > 64) throw Error('SCOPED_MULTILINE_BOUNDED_EXEC');
  ...
  for (const [label,sep,newline] of separators.slice(0,6)) {
    ...
    for(const fold of [false,true]) for(const slow of [false,true]) {
      for(const grammar of ['', 'u', 'v']) {
        ...
        const flags=(fold?'i':'')+grammar, input=string(subject);
        for(let repeat=0;repeat<2;++repeat) {
          const re=makeApi('(?m:(^|$))','dg'+flags,slow);
          ...
          equal(tag+'callback-result',units(input.replace(re,(m,capture,offset,original)=>{
            if(calls.length>16) throw Error('SCOPED_MULTILINE_BOUNDED_CALLBACK');
          ...
          equal(tag+'split',input.split(makeApi('(?m:(^|$))',flags,slow)).map(units),splitPieces.map(selected),tag);
  ...
  equal('fixed-case-count',cases,5082);
  equal('fixed-check-count',checks+1,42104);
  emit(encode({kind:'scoped-multiline-profile',profile,actualWidth:width}));
  emit(encode({kind:'scoped-multiline',profile,
    cases,checks,expectedCases:5082,expectedChecks:42104,
  ...
  if(failureCount!==0) throw Error('SCOPED_MULTILINE_ORACLE: '+failureCount);
})();
